...
inFile = r'C:\Program Files\Pycharm_Projects\Projects_Lab\SCD_System\data\true_data\userAttribute.csv'
userAttribute_df = pd.read_csv(inFile, index_col=0)
inFile2 = r'C:\Program Files\Pycharm_Projects\Projects_Lab\SCD_System\data\true_data\userTrueTrips.csv'
...
userTrip_df.to_csv(r'C:\Program Files\Pycharm_Projects\Projects_Lab\SCD_System\model\output\userPredictionTrips2_6.csv',
                   index=False)
...
for u_id in list(userAttribute_df.index):
    ...
    test_df = userTrips_df2[userTrips_df2['transDate'].map(lambda x: 20170601 <= x <= 20170630)]  # test time
    test_df_ls = test_df['transDate'].tolist()

    # train_df = userTrips_df2[userTrips_df2['transDate'] < 20170801]
    train_df = userTrips_df2[userTrips_df2['transDate'].map(lambda x: x not in test_df_ls)]
    ...
    # print(otherChoose_E_df)
    # print(np.array(otherChoose_E_df))
    # print(otherChoose_E_df2)
    # print(np.array(otherChoose_E_df2))
    ...
    for testday in testday_ls:
        ...
        for i in range(11,49):
            ...
            if i == 11:
                ...
                lastTrueState = result_df.iloc[i - 12,1]
                # ==== modify to compare with other model ====
                nextTrueState = result_df.iloc[i - 11,1]
                if nextTrueState in ['Work-to-Home', 'Other-to-Home', 'Home-to-Work', 'Other-to-Work',
                                           'Home-to-Other', 'Work-to-Other', 'Other-to-Other', ]:
                    prob_ls = stateTrans_MX[i - 11][state_dic[lastTrueState]]
                    prob_ls[0] = 0
                    prob_ls[3] = 0
                    prob_ls[6] = 0
                    prob_max_ind = max(prob_ls)
                    if prob_max_ind != 0:
                        for key, val in state_dic.items():
                            if val == prob_ls.index(prob_max_ind):
                        ...
                        if bb != 0:
                            ...
                            nextPredictionState = random.choice(['Work-to-Home', 'Other-to-Home', 'Home-to-Work', 'Other-to-Work',
                                           'Home-to-Other', 'Work-to-Other', 'Other-to-Other', ])
                else:
                    continue
                # ==== end to modify ====
            ...
            try:
                if nextPredictionState == 'Home-to-Work':
                    ...
                    # switch = 0
                    for t in range(11,i):
                        if result_df.iloc[t - 11,1] in ['Home-to-Other','Work-to-Other','Other-to-Other']:
                            tripPrediction_ls[0] = result_df.iloc[t - 11,3]
                            # switch = 1
            ...
            if nextPredictionState in ['Work-to-Home','Other-to-Home','Home-to-Work','Other-to-Work','Home-to-Other','Work-to-Other','Other-to-Other',]:
                ...
                trip_P_df.to_csv(r'C:\Program Files\Pycharm_Projects\Projects_Lab\SCD_System\model\output\userPredictionTrips2_6.csv',mode='a',index=False,header=False)
        ...
        accrate_state_everyday_ls.append(acc_num / everyday_trip_num)
...
for i in range(len(userAttribute_df)):
    if userAttribute_df.iloc[i, 0] == 'commuter':
        if userAttribute_df.iloc[i,9] != 0:  # 除去 accuracy=0 的异常值
...
print('****All user average accuracy****')
print(userAttribute_df[userAttribute_df.iloc[:, -3].map(lambda x: x != 0)].iloc[:, -3].mean())
userAttribute_df.to_csv(r'C:\Program Files\Pycharm_Projects\Projects_Lab\SCD_System\model\output\userAttribute(accuracy)2_6.csv')
